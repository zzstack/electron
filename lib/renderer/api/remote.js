  ...
  for (let member of members) {
    ...
    if (member.type === 'method') {
      const remoteMemberFunction = function (...args) {
        ...
        if (!isConstructCall && member.returnType === 'void') {
          ipcRenderer.send(command, contextId, metaId, member.name, wrapArgs(args))
          return
        }
  ...
  if (meta.type in types) {
    ...
    if (meta.type === 'function') {
      let remoteFunction = function (...args) {
        ...
        if (!isConstructCall && meta.returnType === 'void') {
          ipcRenderer.send(command, contextId, meta.id, wrapArgs(args))
          return
        }
